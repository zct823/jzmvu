...
+ (void)addContactToDevice:(AddContactAction *)obj
{
//    AddContactAction *obj = [[[self alloc] init] autorelease];
    ...
    NSRange commaRange = [obj.name rangeOfString:@","];
    if (commaRange.location != NSNotFound) {
        NSString *lastName = [[obj.name substringToIndex:commaRange.location]
                              stringByTrimmingCharactersInSet:whitespaceSet];
        ABRecordSetValue(person, kABPersonLastNameProperty, lastName, error);
        NSArray *firstNames = [[[obj.name substringFromIndex:commaRange.location + commaRange.length]
                                stringByTrimmingCharactersInSet:whitespaceSet]
        ...
        NSArray *nameParts = [obj.name componentsSeparatedByCharactersInSet:whitespaceSet];
        int nParts = nameParts.count;
        if (nParts == 1) {
            ABRecordSetValue(person, kABPersonFirstNameProperty, obj.name, error);
        } else if (nParts >= 2) {
    ...
    if (obj.note) {
        ABRecordSetValue(person, kABPersonNoteProperty, obj.note, error);
    }
    
    if (obj.organization) {
        ABRecordSetValue(person, kABPersonOrganizationProperty, (CFStringRef)obj.organization, error);
    }
    
    if (obj.jobTitle) {
        ABRecordSetValue(person, kABPersonJobTitleProperty, (CFStringRef)obj.jobTitle, error);
    }
    
    if (obj.phoneNumbers && obj.phoneNumbers.count > 0) {
        // multi-values: nultiple phone numbers
        ABMutableMultiValueRef phoneNumberMultiValue =
        ABMultiValueCreateMutable(kABStringPropertyType);
        for (NSString *number in obj.phoneNumbers) {
    ...
    if (obj.email) {
        ...
        ABMultiValueCreateMutable(kABStringPropertyType);
        ABMultiValueAddValueAndLabel(emailMultiValue, obj.email,
                                     kABHomeLabel, NULL);
    ...
    if (obj.urlString) {
        ...
        ABMultiValueCreateMutable(kABStringPropertyType);
        ABMultiValueAddValueAndLabel(urlMultiValue, obj.urlString,
    ...
    if (obj.address) {
        ...
        NSArray *parts =
        [obj.address componentsSeparatedByCharactersInSet:
...
//    unknownPersonViewController.unknownPersonViewDelegate = self;
    ABAddressBookAddRecord(iPhoneAddressBook, person, error);
...
//      [unknownPersonViewController dismissModalViewControllerAnimated:YES];
//  }
//}
